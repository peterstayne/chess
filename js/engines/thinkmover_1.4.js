...
game.engines.thinkMover_1_4 = {
	name: 'thinkMover_1_4',
	displayName: 'thinkMover 1.4',
		...
		var moveGen = function(thisnode) {
			...
			var kingpos;
		 	...
		 	for(var i = 0; i < 64; i++) {
				...
				curX = game.coords[i][0];
				curY = game.coords[i][1];
		...
		function searchNode( curnode, ply, qsq ) {
			...
			if(ply === maxply) {
				...
				if(ob[posString]) {
					...
					return { move: [ ob[posString][obIndex][1], ob[posString][obIndex][2] ], output: '<span class="engine-name">' + this.name + '</span> Opening book move.' };
				}
			}
			if(mllength !== 0) {
