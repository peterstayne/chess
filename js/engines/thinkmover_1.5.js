...
var makePositionString = function(thisnode) {
	...
	return positionString;
			...
			var kingpos;
		 	...
		 	for(var i = 0; i < 64; i++) {
				...
				case BLACK_KNIGHT:
					if(curY > 1) {
						if(curY > 2) {
							if(curX > 1 && (tP[i-17] === 0 || tP[i-17] > 6)) movelist.push([i, i-17]);
							if(curX < 8 && (tP[i-15] === 0 || tP[i-15] > 6)) movelist.push([i, i-15]);
						}
						if(curX < 7 && (tP[i-6] === 0 || tP[i-6] > 6)) movelist.push([i, i-6]);
						if(curX > 2 && (tP[i-10] === 0 || tP[i-10] > 6)) movelist.push([i, i-10]);
					}
					if(curY < 8) {
						if(curY < 7) {
							if(curX > 1 && (tP[i+15] === 0 || tP[i+15] > 6)) movelist.push([i, i+15]);
							if(curX < 8 && (tP[i+17] === 0 || tP[i+17] > 6)) movelist.push([i, i+17]);
						}
						if(curX < 7 && (tP[i+10] === 0 || tP[i+10] > 6)) movelist.push([i, i+10]);
						if(curX > 2 && (tP[i+6] === 0 || tP[i+6] > 6)) movelist.push([i, i+6]);
					}
					break;
				...
				case BLACK_BISHOP:
					if(curX !== 1 && curY !== 1) {
						...
						while(cX-- > 1 && cY-- > 1) {
							cS -= 9;
							if(tP[cS] === 0 || tP[cS] > 6) movelist.push([i, cS]);
							if(tP[cS] !== 0) break;
						}
					}
					if(curX !== 8 && curY !== 1) {
				...
				case BLACK_KING: // black king
					...
					if(curX > 1) {
						...
						if(tP[i-9] === 0 || tP[i-9] > 6) movelist.push([i, i-9]);
						if(tP[i+7] === 0 || tP[i+7] > 6) movelist.push([i, i+7]);
					}
					if(tP[i-8] === 0 || tP[i-8] > 6) movelist.push([i, i-8]);
					if(tP[i+8] === 0 || tP[i+8] > 6) movelist.push([i, i+8]);
					if(curX < 8) {
						if(tP[i+1] === 0 || tP[i+1] > 6) movelist.push([i, i+1]);
						if(tP[i-7] === 0 || tP[i-7] > 6) movelist.push([i, i-7]);
		...
		var squareInCheck = function(curnode, square, color) {
			...
			if(coords[square] === undefined) debugger;
...
onmessage = function(e) {
	...
	var resultMove = searchNode(curnode, maxply, false);
	postMessage(resultMove);
};

if(typeof game === 'object') {
	game.engines.thinkMover_1_5 = {
		name: 'thinkMover_1_5',
		displayName: 'thinkMover 1.5',
		type: 'async',
		requestMove: function(curnode) {
			...
			for(var i = 0; i < maxworkers; i++) {
				...
				workerpool[i] = new Worker('/chess/js/engines/thinkmover_1.5.js');
